...
**A landing zone with NO BAR AT ALL.** `claude-config/tools/lab/` is the working example (16 files when this was written, 2026-09-01). Nothing is required to put something there. That matters more than it looks: the failure mode is work being LOST, so any bar at entry costs you tools. **Dusty is fine, lost is not.** A scruffy script somebody hesitates to write up is a script that gets deleted.

**THE LANDING ZONE OWES ONE THING: AN INDEX, KEPT CURRENT BY A TEST.** Every lab carries an index, its README, with a row per tool saying what question the tool answers. A test fails when a tool lands without a row, or when a row outlives its tool. A tool nobody can find is lost in place, so without the index "dusty is fine, lost is not" fails the other way: the next seat checks for prior art, finds nothing, and builds it again. Joe, 2026-09-14: *"if the methodology covers the labs it should say an idex is required and must be maintained"*, *"those indexes are what i hope will keep yall from duplicating effort"*, and *"the idea is once i start working out proper prompting i suspect you will all stop rebuilding the same tools over and over"*.

So: **before writing a tool, read the index.** And the index is held by a test, not by memory, because one kept by hand goes stale the first time someone is in a hurry. On 2026-09-14 claude-config's listed 51 of 123 tools, and projectbook's was 12 behind. The checks: `tests/test_lab_index.py` here, `tests/lab-index-current.py` in projectbook, `tools/test-lab-index-current.py` in claude-config. The row is not a bar at entry. It is one line, written by the person who is still there, and it does not ask the tool to be good.

**A STANDARD MET BY COPYING IS WORTH A GENERATOR.** Joe, 2026-09-14: *"for 2.9 we should cover generators, if we have standards for the tools to avoid geenral annoyances and generators thta mak meeting htose standards easier well all the better."* The standards here (the index row, the three-field header below, the test shape the fleet has converged on) are cheap for one tool and expensive by the dozen, because every seat retypes them. On 2026-09-14 about fifteen `checks/*_test.py` in dynatext-tools were written to one shape by four seats copying an earlier test: a verdict-first check line a mutant can be aimed at, a fixture the test writes itself, a control beside any check that could pass on broken code, and three wiring edits (the CI step, the coverage script, the index row). No generator existed. Every seat retyped the same helper and the same wiring, and the CI step's count line conflicted on every landing.

**A generator writes the shape, never the substance.** Joe, 2026-09-04, on the first one: *"make the generator interfaces. Dont allow implementation?"* The ones in use:

- `claude-config/tools/lab/gen-suite.py` emits a test suite's interface for a Python module, and `projectbook/vm/gen-suite.py` the same contract for a shell suite run end to end. Neither writes the assertions. Both carry positive and negative control slots that are required, not optional, and the generated file FAILS until it is filled in: an unfilled stub never passes.
- `claude-config/tools/lab/lab-index-rows.py` and dynatext-tools' `checks/index_current_test.py --missing` write a lab index's missing rows from each tool's own first docstring line, so keeping the index is one command, and the index check says when it has not been run.
- `projectbook/vm/build-all.py --init` writes a `.projectbook` from a template and marks it as its own output, so it only ever rewrites what it wrote and names a hand-written one instead of touching it.
- dynatext-tools' `dtman.py` writes man pages from a tool's docstrings and argparse calls, so the documentation is generated from the code it describes.

So: **when a standard is being met by copying, write the generator, and keep it to what is the same every time.** What differs every time, the assertion, the control, the reason, stays with the person, because a generator that fills those in writes a test that cannot fail. A generator is a tool like any other here: it lands in the lab with the three-field header, and its index row names the standard it makes cheap.
...
2. **A killed process runs no cleanup at all, so whatever kills a run cleans up after it.** Joe, on that point: *"yeah, thats what happens when you kill a run, wipe your ass"*. A harness with a timeout should give each run its own `TMPDIR` and remove it afterwards, whatever happens. `atexit` cannot help once the process is gone, and every leftover sunblade2000 found that night came from a killed run: out of memory, or a timeout.
...
2. **Record the baseline the same way the gate measures, and hold the line at
   ...
   **Name it `quality-baseline.json` at the repo root.** The pre-commit hook
   ...
   `install-hooks.py` named it zero times. Written, merged, and not running.
