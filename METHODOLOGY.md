...
That third line is the one that earns its place, and it is written naturally rather than under duress. Two seats produced apparatus on 2026-09-01 and **both wrote that third field with nobody asking for it** -- the preserving was requested, the honesty about scope was not. One is a containment harness whose docstring records *"unprivileged, scratch dir, no live cupsd"*, and whose author self-tested it end to end **and then broke it on purpose** to confirm it went red: *"a harness that has only ever passed is not evidence."*

**And the same docstring records four traps, none of which were mistakes its author made.** They were identified by reading the classifier before building: fixture files must carry the magic bytes or they classify as unrecognised and prove nothing; the plain file is the control rather than the test; the hard link needs its other name outside the tree or you cannot tell unlinked from destroyed; a symlinked DIRECTORY is a separate case from a symlinked FILE. What the next person would get wrong, cheaper to write down than to re-derive.

That distinction is load-bearing and this section had it wrong in an earlier draft, which said "three wrong attempts, named". Its author corrected it: **"wrong attempts" implies the third field is earned through failure, which quietly raises the bar this section spends its length lowering.** Somebody with no failures to confess concludes they have nothing to write, and says nothing. Analysis written down counts.
